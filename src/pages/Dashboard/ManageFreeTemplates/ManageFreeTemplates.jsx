...
const ManageFreeTemplates = () => {
    const [freeTemplates, , refetch] = useFreeTemplate();
    ...
    const handleDeleteItem = (template) => {
        Swal.fire({
            title: "Are you sure?",
            ...
            if (result.isConfirmed) {
                const res = await axiosSecure.delete(`/free/${template._id}`);
    ...
    const totalPages = useMemo(() => Math.ceil(filteredTemplates.length / TEMPLATES_PER_PAGE), [filteredTemplates]);
            ...
            <div>
                <div className="text-center mb-4 md:mb-10">
                    <form onSubmit={handleSearch} className="flex flex-col items-center md:flex-row md:justify-center">
                        <input type="text" name="search" className="input input-bordered text-xs md:text-base mb-2 md:mb-0 md:mr-1" placeholder="Template Search" />
                    ...
                    <table className="table w-full">
                        ...
                        <tbody>
                            {currentItems.map((template, index) => (
                                <tr key={template._id}>
                                    <td>{(currentPage - 1) * TEMPLATES_PER_PAGE + index + 1}</td>
                                    <td>
                ...
                <div className="block md:hidden">
                    {currentItems.map((template, index) => (
                        <div key={template._id} className="card bg-base-100 shadow-xl mb-3 p-2 mr-2">
                            ...
                            <div className="card-body p-2">
                                ...
                                <div className="card-actions justify-end mt-1">
                                    ...
                                    <button onClick={() => handleDeleteItem(template)} className="btn btn-danger btn-xs">
                                        <FaTrashAlt />
                                    </button>
                                </div>
                            </div>
